...
const Fackwork = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://facultyrecruitmentandmonitoringsystem-41bq.onrender.com/wofaculty/${id}`, {
          method: 'GET',
          headers: {
  ...
  const PostData = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`https://facultyrecruitmentandmonitoringsystem-41bq.onrender.com/wfaculty/${id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  return (
    ...
    />
      ...
      <strong>
        ...
        <em>
          <h2>
          Details of Faculty who are working in the Institute
            </h2></em>
        </center>
      </strong>
      
        <div className="table-responsive">
          <table style={{ width: '100%' }} className="text-dark table">
